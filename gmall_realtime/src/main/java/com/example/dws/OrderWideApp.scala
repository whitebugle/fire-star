...
import com.example.bean.{OrderDetail, OrderInfo, OrderWide}
import com.example.util.{MyKafkaSink, MyKafkaUtil, MyRedisUtil, OffsetManagerUtil}
   ...
   val orderWideSplitDStream: DStream[OrderWide] = orderWideDStream.mapPartitions {
     orderWideItr => {
       ...
       for (orderWide <- orderWideList) {
         ...
         //3.3判断是否为最后一条  计算实付分摊
         //（当前订单金额  == 原订单总金额  -  原订单累加金额 ） 说明就是最后一条订单
         if (detailAmount == orderWide.original_total_amount - orderOriginSum) {
           ...
           /*
           原订单总金额 / 计算当前订单金额 = 实付总金额 / 最后分摊金额
           变形 ------>
           最后分摊金额 = 实付总金额 * 计算当前订单金额  / 原订单总金额
            */
   ...
   orderWideSplitDStream.foreachRDD{
     rdd=>{
       ...
       df.write.mode(SaveMode.Append)
         ...
         .jdbc("jdbc:clickhouse://hadoop01:8123/default","t_order_wide_2021",new Properties())
