...
import com.example.bean.{OrderDetail, SkuInfo}
import com.example.util.{MyKafkaSink, MyKafkaUtil, OffsetManagerUtil, PhoenixUtil}
 ...
 * create by young
 * date:21/6/6
 * desc: 从Kafka的 ods_order_detail主题中，读取订单明细数据
 */
  ...
  def main(args: Array[String]): Unit = {
    ...
    val orderDetailWithSkuDStream: DStream[OrderDetail] = orderDetailDstream.mapPartitions {
      orderDetailItr => {
        ...
        var sql: String = s"select id ,tm_id,spu_id,category3_id,tm_name ,spu_name,category3_name from gmall2021_sku_info where id in('${skuIdList.mkString("','")}')"
