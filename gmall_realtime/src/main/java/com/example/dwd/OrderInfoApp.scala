...
import com.example.bean.{OrderInfo, ProvinceInfo, UserInfo, UserStatus}
...
object OrderInfoApp {
  ...
  def main(args: Array[String]): Unit = {
   ...
   // orderInfoDStream.print(1000)
    ...
    orderInfoWithFirstFlagDStream.cache()
    orderInfoWithFirstFlagDStream.print(20)
    ...
    */
    ...
    val orderInfoWithProvinceDStream: DStream[OrderInfo] = orderInfoRealDStream.mapPartitions {
      orderInfoItr => {
        ...
        var sql: String = s"select id,name,area_code,iso_code from gmall2021_province_info where id in('${provinceIdList.mkString("','")}')"
    ...
    val orderInfoWithProvinceDStream: DStream[OrderInfo] = orderInfoRealDStream.transform {
      rdd => {
        //从Phoenix中查询所有的省份数据
        var sql: String = "select id,name,area_code,iso_code from gmall2021_province_info"
        val provinceInfoList: List[JSONObject] = PhoenixUtil.queryList(sql)
    ...
    val orderInfoWithUserInfoDStream: DStream[OrderInfo] = orderInfoWithProvinceDStream.mapPartitions {
      ...
      orderInfoItr => {
        ...
        var sql: String = s"select id,user_level,birthday,gender,age_group,gender_name from gmall2021_user_info where id in ('${userIdList.mkString("','")}')"
    ...
    orderInfoWithUserInfoDStream.cache()
    ...
    //orderInfoWithFirstFlagDStream    orderInfoWithUserInfoDStream
...
//        rdd.cache()
        ...
        rdd.foreachPartition{
          orderInfoItr=>{
            ...
            MyESUtil.bulkInsert(orderInfoList, "gmall2021_order_info_" + dateStr)
