...
import com.example.util.{MyKafkaUtil, OffsetManagerM}
          ...
          DB.localTx {
            implicit session =>{
              ...
              for (offsetRange <- offsetRanges) {
                ...
                println("偏移量提交执行")
                SQL("replace into offset_2021  values(?,?,?,?)").bind(groupId, topic, partitionId, untilOffset).update().apply()
              }
            }
          }
        }
        ...
        if (tmSumArr !=null && tmSumArr.size > 0) {
          ...
          DB.localTx {
            implicit session =>{
              ...
              //val params: Seq[Seq[Any]] = Seq(Seq("2021-06-01 10:10:10","101","品牌1",2000.00),
              // Seq("2021-06-01 10:10:10","102","品牌2",3000.00))
              ...
              for (offsetRange <- offsetRanges) {
                ...
                val untilOffset: Long = offsetRange.untilOffset
                println("执行更新偏移量")
                SQL("replace into offset_2021  values(?,?,?,?)").bind(groupId, topic, partitionId, untilOffset)
                  .update().apply()
              }
          /*    //批量插入偏移量
              val batchOffsetList: ListBuffer[Seq[Any]] = ListBuffer[Seq[Any]]()
              for (offsetRange <- offsetRanges) {
                val partitionId: Int = offsetRange.partition
                val untilOffset: Long = offsetRange.untilOffset
                batchParamsList.append(Seq(groupId,topic,partitionId,untilOffset))
              }
              SQL("replace into offset_2021  values(?,?,?,?)").batch(batchOffsetList.toSeq:_*).apply()*/
            }
          }
        }
      }
