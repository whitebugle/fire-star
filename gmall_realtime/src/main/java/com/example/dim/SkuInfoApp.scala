...
import com.example.bean.SkuInfo
import com.example.util.{MyKafkaUtil, OffsetManagerUtil, PhoenixUtil}
...
object SkuInfoApp {
  def main(args: Array[String]): Unit = {
    ...
    val skuInfoDstream: DStream[SkuInfo] = objectDstream.transform {
      rdd => {
        //tm_name
        val tmSql = "select id ,tm_name  from gmall2021_base_trademark"
        ...
        //category3
        val category3Sql = "select id ,name from gmall2021_base_category3" //driver  周期性执行
        ...
        // spu
        val spuSql = "select id ,spu_name  from gmall2021_spu_info" // spu
    ...
    skuInfoDstream.cache()
    skuInfoDstream.print()
    ...
    skuInfoDstream.foreachRDD {
      rdd => {
        rdd.saveToPhoenix(
          ...
          new Configuration,
          Some("hadoop102,hadoop03,hadoop104:2181"))
